...
const statusRank = (status) =>
  ({ backlog: 0, ready: 1, in_progress: 2, blocked: 2, review: 3, done: 4 })[
    status
  ];
function clearDrag() {
  draggedTicket = null;
...
async function dropTicket(status) {
  ...
  clearDrag();
  if (!ticket || saving || ticket.status === status) {
    refresh();
    return;
  }
...
function render() {
  ...
  const live = board
    ? board.tickets.filter((t) => !t.trashed && !t.archived).length
    : 0;
  $("summary").textContent = board
    ? `${live} ${live === 1 ? "ticket" : "tickets"}`
    : "No board yet. Create one to plan work with your agents.";
  $("create").textContent = board ? "New ticket" : "Create board";
  $("create").disabled = saving;
  $("tasks-link").href = `/?project=${encodeURIComponent(project)}`;
  $("tasks-page-link").href = $("tasks-link").href;
  ...
  if (signature !== rendered) {
    ...
    $("completed-summary").textContent =
      `Completed (${board?.tickets.filter((t) => t.archived).length || 0})`;
    const trashCount = board?.tickets.filter((t) => t.trashed).length || 0;
    $("trash").hidden = !trashCount;
    $("trash-summary").textContent = `Trash (${trashCount})`;
    if (board)
      for (const status of [
        ...columns.filter((s) => s !== "backlog"),
        "backlog",
        "completed",
        "trash",
      ]) {
        const tickets = board.tickets.filter((t) =>
          status === "trash"
            ? t.trashed
            : status === "completed"
              ? t.archived
              : !t.trashed && !t.archived && t.status === status,
        );
        ...
        column.ondragover = (event) => {
          if (
            !draggedTicket ||
            saving ||
            ["trash", "completed"].includes(status) ||
        ...
        column.ondrop = (event) => {
          event.preventDefault();
          if (!["trash", "completed"].includes(status)) dropTicket(status);
        };
        const heading = element(
          "h2",
          status === "completed" ? "Archived from Done" : label(status),
        );
        heading.append(element("span", tickets.length));
        column.append(heading);
        const list = element("div", "", "ticket-list");
        column.append(list);
        for (const ticket of tickets) {
          ...
          const card = element(
            "article",
            "",
            isRow ? "kanban-row" : "kanban-card",
          );
          const open = element("button", "", "kanban-card-open");
          open.type = "button";
          card.draggable = !ticket.trashed && !ticket.archived;
          card.ondragstart = (event) => {
            if (saving || $("editor").open) {
              event.preventDefault();
              return;
            }
            draggedTicket = structuredClone(ticket);
            generation++;
          ...
          open.append(
            element("strong", ticket.title),
            element("span", ticket.assignee || "Unassigned"),
          );
          if (ticket.feedback)
            open.append(
              element("span", "Feedback needs attention", "needs-feedback"),
            );
          open.append(
          ...
          if (ticket.trashed)
            shortcut("Restore", () => quickAction(ticket, "restore_ticket"));
          else if (ticket.archived)
            shortcut("Restore to Done", () =>
              quickAction(ticket, "unarchive_ticket"),
            );
          else if (ticket.status === "backlog") {
            shortcut("To board", () =>
              quickAction(ticket, "move_ticket", {
                status: "ready",
                reason: null,
              }),
            );
            shortcut(
              "Trash",
              () => quickAction(ticket, "trash_ticket"),
              "danger",
            );
          } else if (ticket.status === "done")
            shortcut("Archive", () => quickAction(ticket, "archive_ticket"));
          if (actions.childElementCount) card.append(actions);
          list.append(card);
        }
        if (!tickets.length)
          column.append(element("p", "No tickets", "empty-column"));
        $(
          status === "backlog"
            ? "backlog"
            : status === "trash"
...
async function refresh() {
  if (saving || refreshing || draggedTicket) {
    pending = true;
    return;
  }
  refreshing = true;
  ...
  try {
    ...
    if (previous)
      project = projects.find((p) => p.id === previous.id)?.name || project;
    ...
    const next = await api(
      `/api/kanban?project=${encodeURIComponent(project)}`,
    );
    if (requestGeneration !== generation) return;
    board = next;
    render();
    ...
    if (requestGeneration === generation) message("error", error.message);
  } finally {
    refreshing = false;
    if (pending) {
      pending = false;
...
function setDisabled() {
  ...
  for (const id of [
    ...
    $(id).disabled =
      saving ||
      stale ||
      ((editorTicket?.trashed || editorTicket?.archived) &&
        id !== "trash-ticket" &&
        id !== "archive-ticket");
}
function openEditor(nextMode, ticket = null) {
  ...
  $("editor-heading").textContent =
    mode === "board"
      ...
      : ticket
        ? "Ticket details"
        : "New ticket";
  $("board-fields").hidden = mode !== "board";
  $("ticket-fields").hidden = mode === "board";
  $("ticket-actions").hidden = !ticket;
  $("board-name").value = "Delivery";
  ...
  if (ticket) {
    ...
    $("to-board").hidden =
      ticket.status !== "backlog" || ticket.trashed || ticket.archived;
    $("trash-ticket").hidden =
      !ticket.trashed && (ticket.status !== "backlog" || ticket.archived);
    $("archive-ticket").hidden = ticket.status !== "done" || ticket.trashed;
    $("archive-ticket").textContent = ticket.archived
      ? "Restore to Done"
      : "Archive";
    $("trash-ticket").textContent = ticket.trashed ? "Restore" : "Trash";
    $("feedback").classList.toggle("needs-feedback", Boolean(ticket.feedback));
    $("feedback").textContent = ticket.feedback
      ? `Outstanding feedback\n${ticket.feedback}`
      : "No outstanding move feedback.";
    ...
    $("activity").replaceChildren(
      ...ticket.activity
        ...
        .map((event) => {
          const item = element(
            "li",
            `${label(event.action)}${event.from ? `: ${label(event.from)} → ${label(event.to)}` : ""}${event.body ? `\n${event.body}` : ""}`,
          );
          item.append(
            element(
              "small",
...
async function mutate(action, fields) {
  ...
  const draft = Object.fromEntries(
    ["title", "description", "assignee", "reason", "comment", "resolution"].map(
      (id) => [id, $(id).value],
    ),
  );
  saving = true;
  ...
  setDisabled();
  try {
    const result = await api("/api/kanban", {
      method: "POST",
      body: JSON.stringify({ project, actor: "user", action, ...fields }),
    });
    board = result;
    message("form-error", "");
    ...
    if (
      ...
      openEditor(
        "ticket",
        board.tickets.find((t) => t.id === id),
      );
      const submitted =
        {
          edit_ticket: ["title", "description", "assignee"],
          move_ticket: ["reason"],
          comment: ["comment"],
          address_feedback: ["resolution"],
        }[action] || [];
      for (const [field, value] of Object.entries(draft))
        if (!submitted.includes(field)) $(field).value = value;
    } else $("editor").close();
    render();
  } catch (error) {
    message("form-error", error.message);
...
$("trash-ticket").onclick = () =>
  mutate(
    editorTicket.trashed ? "restore_ticket" : "trash_ticket",
    ticketFields(),
  );
const ticketFields = () => ({
  id: editorTicket.id,
  expected_revision: editorTicket.revision,
});
...
$("reload-ticket").onclick = () => {
  const ticket = board?.tickets.find((t) => t.id === editorTicket?.id);
  if (ticket) openEditor("ticket", ticket);
  else $("editor").close();
};
$("ticket-form").onsubmit = (event) => {
  event.preventDefault();
  if (mode === "board")
    return mutate("create_board", { name: $("board-name").value });
  mutate(editorTicket ? "edit_ticket" : "create_ticket", {
    ...(editorTicket ? ticketFields() : {}),
    title: $("title").value,
...
$("add-comment").onclick = () =>
  mutate("comment", { ...ticketFields(), body: $("comment").value });
$("resolve").onclick = () =>
  mutate("address_feedback", {
    ...ticketFields(),
...
$("status").replaceChildren(
  ...columns.map((status) => {
    const option = element("option", label(status));
    option.value = status;
    return option;
  }),
);
